# DESCRIPTION
# 44. Number of Natives of Great Britain, By states: 1890
...
# Creating a data frame for # 44
# The data is estimated from the primary plot
states <- c("PENNSYLVANIA", "NEW YORK", "MASSACHUSETTS", "ILLINOIS", "OHIO", "MICHIGAN",
            "NEW JERSEY", "CALIFORNIA", "IOWA", "WISCONSIN", "CONNECTICUT", "UTAH",
            "KANSAS", "RHODE ISLAND", 'MISSOURI', "MINNESOTA", "COLORADO", "NEBRASKA",
            "WASHINGTON", "INDIANA", 'TEXAS')

number <- c(192000, 182000, 100000, 95000, 75000, 70000, 67000, 47000, 33000, 32500, 
            26000, 25500, 25000, 24900, 24800, 24000, 23000, 22000, 21500, 21000,
            20000)
...
#  Plotting Graph 44
plot44 <- ggplot(df, aes(x = number, y = reorder(states, -number))) +
  annotate("segment", x = 0, xend = 0, y = 0, yend = 22.6) +
  annotate("segment", x = seq(0, 250000, 50000), xend = seq(0, 250000, 50000), y = 0, yend = 21.5) +
  annotate("segment", x = -100000, xend = 250000, y = 21.5, yend = 21.5) +
  ...
  labs(title = "44. NUMBER OF NATIVES OF GREAT BRITAIN,
       BY STATES: 1890.",
       subtitle = "STATES.") +
  coord_cartesian(xlim = c(-40000, 225000), ylim = c(1, 22)) +
  labs(y = NULL, x = NULL)  +
...
ggsave("Number of natives of Great Britain, by states - 1890 #44.png",
       plot44, height = 8, width = 8)
